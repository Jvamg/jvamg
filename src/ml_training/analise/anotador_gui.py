...
class LabelingTool(tk.Tk):
    ...
    def _setup_ui(self):
        ...
        self.info_label = tk.Label(frame_controles, text="Carregando...", font=(
            "Arial", 12), justify=tk.LEFT, anchor="w")
    ...
    def setup_dataframe(self, arquivo_entrada, arquivo_saida):
        try:
            ...
            rename_map = {'timeframe': 'intervalo', 'padrao_tipo': 'tipo_padrao',
                          'ombro1_idx': 'data_inicio', 'ombro2_idx': 'data_fim', 'cabeca_idx': 'data_cabeca'}
    ...
    def _validate_hns_pattern(
        ...
        if not self.is_head_extreme(df_historico, cabeca, avg_pivot_dist):
            return None
    ...
    def plotar_grafico_com_zigzag(self):
        ...
        for tentativa in range(Config.MAX_DOWNLOAD_TENTATIVAS):
            try:
                ...
                if 'm' in intervalo:
                    ...
                    periodo_busca = '2y'
                print(
                    f"Buscando dados para {ticker} ({intervalo})... Tentativa {tentativa + 1}/{Config.MAX_DOWNLOAD_TENTATIVAS}")
        ...
        pivots = self._calcular_zigzag_para_plot(
            df_view, params['deviation'], params['min_distance'])
        zigzag_line = self._preparar_zigzag_plot(pivots, df_view.index)
        ad_plot = [mpf.make_addplot(
            zigzag_line, color='blue', width=1.2)] if not zigzag_line.empty else []

        fig, axlist = mpf.plot(df_view, type='candle', style='charles', returnfig=True, figsize=(
            12, 8), addplot=ad_plot, title=f"{ticker} ({intervalo}) - Padrão {self.indice_atual}", warn_too_much_data=10000)
        ax = axlist[0]
        start_pos = df_view.index.get_indexer(
            [data_inicio], method='nearest')[0]
        ...
        if pd.notna(padrao_info['data_cabeca']):
            head_pos = df_view.index.get_indexer(
                [padrao_info['data_cabeca']], method='nearest')[0]
    ...
    def identificar_padroes_hns(self, pivots: List[Dict[str, Any]], df_historico: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for i in range(n - 6):
            ...
            if tipo_padrao:
                ...
                dados_padrao = self._validate_hns_pattern(
    ...
    def _calcular_zigzag_para_plot(self, df, deviation_percent, min_distance_bars):
        if len(df) < 2:
            return []
        pivots = [{'idx': df.index[0],
                   'preco': df['low'].iloc[0], 'tipo': 'VALE'}]
...
if __name__ == '__main__':
    ...
    if not os.path.exists(Config.ARQUIVO_ENTRADA):
        print(
            f"Erro: Arquivo de entrada '{Config.ARQUIVO_ENTRADA}' não encontrado.")
