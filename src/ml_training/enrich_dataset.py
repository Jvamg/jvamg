...
def main():
    ...
    for index, linha in tqdm(df.iterrows(), total=df.shape[0], desc="Enriquecendo Padrões"):
        try:
            ...
            if df_janela.empty:
                print(
                    f"  -> AVISO [Índice {index}]: Não foi possível baixar dados para {linha['ticker']}. Pulando.")
                continue

            df_janela.index = df_janela.index.tz_localize(
                None)  # Remove o fuso horário

            # A mágica acontece aqui: recalculamos a geometria
            features_geometricas = recalcular_geometria_padrao(
                df_janela, linha)
    ...
    total = len(df)
    print(
        f"Sucesso: {sucesso} de {total} padrões foram enriquecidos com sucesso ({sucesso/total:.2%}).")
